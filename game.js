...
function getWorld() {
  ...
  $.ajax({
    url: "game.xml", // name of file you want to parse
    dataType: "xml",
    success: setupWorld,
    error: function(){alert("Error: Something went wrong reading:gamw.xml");}
  });
}
...
class World {
  constructor(map) {
    ...
    map.children().each(function(index) {
      ...
      // Items are stored in an  array, mapping room name to room object
      } else if (tag=="item") {
